...
import 'package:simple_book_log/widget/component/bookshelf/bookshelf_filter/bookshelf_filter_form_button.dart';
...
class BookshelfTemplate extends StatelessWidget {
  ...
  List<Widget> buildBookList(List<BookRow> bookRows) {
    ...
    List<String> initials = items
        .map((ele) {
          if (ele.bookRow.titleKana.trim() == "") {
            return "不明";
          } else {
            return ele.bookRow.titleKana.trim().substring(0, 1);
          }
        })
        .toSet()
        .toList()
    ...
    List<Widget> headerAndItems = initials
        .map(
          (ini) => [
            ...
            ...items.where((item) {
              if (ini == "不明") {
                return item.bookRow.titleKana.trim() == "";
              } else {
                return item.bookRow.titleKana.startsWith(ini);
              }
            }).toList()
          ],
        )
  ...
  Widget build(BuildContext context) {
    ...
    return TemplateSliverScaffold(
      ...
      secondaryFloadingActionButton: BookshelfFilterFormButton(
        accentColor: _sessionCubit.getAccentColor(),
      ),
    );
  }
}
